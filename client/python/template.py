...
# user func
def isConnected(resp : PacketResp, target : ObjType.Player | ObjType.Item | ObjType.Block):
    route.clear()
    for map in resp.data.map:
        if len(map.objs):
            for obj in map.objs: # There may be more than one obj on a grid
                if obj.type == ObjType.Player and obj.property.player_id == gContext["playerID"]:
                    # print(f"Start Search at {map.x},{map.y}\n")
                    return search ([map.x, map.y], [map.x, map.y], resp, target)
    return False
